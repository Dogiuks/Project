...
void MainWindow::on_buttonHome_clicked()
{
    ...
    for(int i=0; i<10; i++){
        for(int j=0; j<10; j++){
            int num = 1+(74*output[i+10*j].demand/100);
            if(num>60)
            {
                red = 255;
                green = 0;
            ...
            ui->warehouseWiew->item(j,i)->setBackgroundColor(QColor(red, green, blue, 255));
            ui->warehouseWiew->item(j,i)->setText(QString::number(num));
